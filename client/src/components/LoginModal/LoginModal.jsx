...
function LoginModal(props) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post("http://52.47.191.228:8080/usuarios/login", JSON.stringify({ correo: email, password }), {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        const res = await axios.get(`http://52.47.191.228:8080/usuarios/user/${email}`)
